...
from ..prediction.predictors import predict_history_shrunk, predict_news_keywords
...
async def run_directional_overlay(
    ...
    for event, market, yes_book in candidates:
        ...
        p_n = predict_news_keywords(case)
        p = (p_h + p_n) / 2.0
        edge = p - ask
        if edge < min_edge:
            continue
        ...
        intent = OrderIntent(
            ...
            metadata={"overlay": True, "p_model": p, "edge_vs_ask": edge},
        )
